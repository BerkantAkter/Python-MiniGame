...
class GamePanel:
    ...
    def __init__(self, grid):
        ...
        self.background = app.Frame(self.root, bg=GamePanel.BACKGROUND_COLOR)
        self.cell_labels = []
        for i in range(self.grid.size):
            row_labels = []
            for j in range(self.grid.size):
                label = app.Label(self.background, text='',
                                 bg=GamePanel.EMPTY_CELL_COLOR,
                                 justify=app.CENTER, font=GamePanel.FONT,
                                 width=4, height=2)
...
class Game:
    ...
    def you_win(self):
        if not self.won:
            self.won = True
            print('KAZANDIN!')
            if messagebox.askyesno('2048', 'TEBRİKLER KAZANDIN\n'
                                       'OYUNA DEVAM ETMEK İSTERMİSİN?'):
                self.keep_playing = True

    def game_over(self):
        print('OYUN BİTTİ!')
        messagebox.showinfo('2048', 'ÜZGÜNÜM\n'
                                    'KAYBETTİN!')
